...
function botRisk(req) {
  ...
  if (/headless|phantomjs|selenium|playwright|puppeteer|webdriver|curl|python-requests|wget/i.test(ua)) {
...
async function audit(session, risk) {
  try {
    ...
    await fetch(UPSTREAM, {
      ...
      body: JSON.stringify({
        action: 'logEvent',
        eventName: risk.score >= 40 ? 'bot_risk_detected' : 'assessment_session_issued',
        ...
        metadata: {
          ...
          signals: risk.signals.slice(0, 6)
        }
      }),
      redirect: 'follow',
    ...
    // Audit logging must never make session issuance crash for a legitimate user.
  }
}
...
module.exports = async function handler(req, res) {
  ...
  // Do not block the user on audit persistence.
  void audit(session, risk);
